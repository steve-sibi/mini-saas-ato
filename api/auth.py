...
def login_post():
    ...
    with SessionLocal() as db:
        ...
        if not u or not bcrypt.verify(pw, u.password_hash):
            logger.info("auth_fail", extra={"evt":"login","outcome":"fail","email":email})
        ...
        if u.mfa_enabled:
            ...
            if not code or not pyotp.TOTP(u.mfa_secret).verify(code, valid_window=1):
                logger.info("mfa_fail", extra={"evt":"login","outcome":"mfa_fail","email":email})
        ...
        logger.info("auth_success", extra={"evt":"login","outcome":"success","email":email,"device_hash":dhash})
...
def enable_mfa():
    ...
    with SessionLocal() as db:
        ...
        u.mfa_secret = secret; u.mfa_enabled = True; db.commit()
